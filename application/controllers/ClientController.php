...
class ClientController extends CI_Controller {
	...
	function update() {					
		...
		$token = bin2hex($token);
		$post_data = $this->input->post();
		$creation = $this->clientmodel->update($post_data,$token);
		if($creation){
			$message =array('status'=>'1','msg'=>'Client Updated successfully.','icon'=>'success',"csrfTokenName" => $this->security->get_csrf_token_name(), "csrfHash" => $this->security->get_csrf_hash());
		}else{
			$message =array('status'=>'0','msg'=>'Somthing Went Wrong.','icon'=>'danger',"csrfTokenName" => $this->security->get_csrf_token_name(), "csrfHash" => $this->security->get_csrf_hash());
		}
		echo json_encode($message);
