...
class DepartmentController extends CI_Controller {
    ...
    function create() {					
		...
		$creation = $this->departmentmodel->create($post_data,$token);
		if($creation){
			$message =array('status'=>'1','msg'=>'Department Created successfully.','icon'=>'success',"csrfTokenName" => $this->security->get_csrf_token_name(), "csrfHash" => $this->security->get_csrf_hash());
		}else{
	...
	function update() {					
		...
		$token = bin2hex($token);
		$post_data = $this->input->post();
		$creation = $this->departmentmodel->update($post_data,$token);
		if($creation){
			$message =array('status'=>'1','msg'=>'Department Updated successfully.','icon'=>'success',"csrfTokenName" => $this->security->get_csrf_token_name(), "csrfHash" => $this->security->get_csrf_hash());
		}else{
			$message =array('status'=>'0','msg'=>'Somthing Went Wrong.','icon'=>'danger',"csrfTokenName" => $this->security->get_csrf_token_name(), "csrfHash" => $this->security->get_csrf_hash());
		}
		echo json_encode($message);
	...
	function delete() {					
		...
		$token = bin2hex($token);
		$post_data = $this->input->post();
		$delection = $this->departmentmodel->delete($post_data,$token);
		if($delection){
			$message =array('status'=>'1','msg'=>'Department Deleted successfully.','icon'=>'success',"csrfTokenName" => $this->security->get_csrf_token_name(), "csrfHash" => $this->security->get_csrf_hash());
		}else{
			$message =array('status'=>'0','msg'=>'Somthing Went Wrong.','icon'=>'danger',"csrfTokenName" => $this->security->get_csrf_token_name(), "csrfHash" => $this->security->get_csrf_hash());
		}
		echo json_encode($message);
