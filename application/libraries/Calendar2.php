...
class Calendar2 {
    ...
    public function render_booking_calendar_new($equipment_id, $month, $year, $calendar_days, $equipment_calendar, $equipment_booking_calendar, $booking_request_calendar) {
        $dateObj = DateTime::createFromFormat('!m', $month);
        $monthName = $dateObj->format('F');
        ...
        /* added by Natesh 02-Aug-2019 - To Highlight todays date */
        $today_year = date('Y');
        $today_month = date('m');
        $today_day = date('j');
        /*         * ************************ */
        ...
        $nextbookingtoken = $this->CI->validation->gen_token(array($equipment_id, $next_month, $next_year, "Book"));
        $previousbookingtoken = $this->CI->validation->gen_token(array($equipment_id, $previous_month, $previous_year, "Book"));
        /*         * *************** */

        $calendar_html = "";
        $calendar_html = $calendar_html . '<div class="row" style="border-bottom:1px solid #9e9e9e">';
        $calendar_html = $calendar_html . '<div class="col-md-1" style="float:left; font-size:1.3em; text-align:center; height:40px; line-height:40px; color:#757575"><a href="' . $this->CI->config->item('base_url') . 'master/equipment/booking/' . $previous_month . '/' . $previous_year . '/' . $equipment_id . '/' . $previousbookingtoken . '"><i class="fa fa-angle-left" style="float:left; font-size:1.3em; text-align:center; height:40px; line-height:40px; color:#757575" aria-hidden="true" data-toggle="tooltip" title="Prev month"></i></a></div>';
        ...
        $calendar_html = $calendar_html . '</div>';
        $calendar_html = $calendar_html . '<div class="col-md-1" style="float:left; font-size:1.3em; text-align:center; height:40px; line-height:40px; color:#757575"><a href="' . $this->CI->config->item('base_url') . 'master/equipment/booking/' . $next_month . '/' . $next_year . '/' . $equipment_id . '/' . $nextbookingtoken . '"><i class="fa fa-angle-right" style="float:left; font-size:1.3em; text-align:center; height:40px; line-height:40px; color:#757575" aria-hidden="true" data-toggle="tooltip" title="Next month"></i></a></div>';
        $calendar_html = $calendar_html . '</div>';
        if (isset($calendar_days)) {
            $calendar_html = $calendar_html . '<div class="row">';
            $calendar_html = $calendar_html . '<div style="float:left; width:14.28%; font-size:1.1em; font-weight:400; text-align:center; height:50px; line-height:50px; color:red;" data-toggle="tooltip" title="Sunday">S</div>';
            ...
            $calendar_html = $calendar_html . '<div style="float:left; width:14.28%; font-size:1.1em; font-weight:400; text-align:center; height:50px; line-height:50px; color:red;" data-toggle="tooltip" title="Saturday">S</div>';
            $calendar_html = $calendar_html . '</div>';
            ...
            for ($row = 0; $row < 6; $row++) {
                ...
                for ($col = 0; $col < 7; $col++) {
                    if (isset($calendar_days[$row][$col])) {
                        ...
                        /* added by Natesh 02-Aug-2019 - To Highlight todays date */
                        if ($day_number == $today_day && $cur_month == $today_month && $cur_year == $today_year) {
                            $today_markup = " todays-date-markup";
                        } else {
                            $today_markup = "";
                        }
                        /*                         * ********************* */
                        ...
                        if ($down_date) {
                            if ($col == 0 || $col == 6) {
                                $calendar_html = $calendar_html . '<div style="float:left; width:14.28%; text-align:center; height:50px; line-height:50px; color:red;"><div class="day-down' . $today_markup . '" data-toggle="tooltip" title="' . $down_reason . '">' . $day_number . '</div></div>';
                            } else {
                                $calendar_html = $calendar_html . '<div style="float:left; width:14.28%; text-align:center; height:50px; line-height:50px; color:#9e9e9e;"><div class="day-down' . $today_markup . '" data-toggle="tooltip" title="' . $down_reason . '">' . $day_number . '</div></div>';
                            }
                        } else {
                            if ($col == 0 || $col == 6) {
                                $calendar_html = $calendar_html . '<div style="float:left; width:14.28%; text-align:center; height:50px; line-height:50px; color:red; "><div class="day-up' . $today_markup . '" data-dd="' . $day_number . '" data-dm="' . $month . '" data-dy="' . $year . '" data-token="' . md5($day_number . $month . $year) . '" data-dt="' . $day_number . '-' . substr($monthName, 0, 3) . '-' . $year . '">';
                            } else {
                                $calendar_html = $calendar_html . '<div style="float:left; width:14.28%; text-align:center; height:50px; line-height:50px; color:#9e9e9e"><div class="day-up' . $today_markup . '" data-dd="' . $day_number . '" data-dm="' . $month . '" data-dy="' . $year . '" data-token="' . md5($day_number . $month . $year) . '" data-dt="' . $day_number . '-' . substr($monthName, 0, 3) . '-' . $year . '">';
                            }
                            $calendar_html = $calendar_html . $day_number;

                            if ($bookings)
                                $calendar_html = $calendar_html . "<span style='color:red' data-toggle='tooltip' title='" . $bookings_count . " Booking(s)'><i style='font-size:0.4em; line-height:30px; vertical-align:top' class='fa fa-circle' aria-hidden='true'></i><span>";
                            if ($requests)
                                $calendar_html = $calendar_html . "<span style='color:orange' data-toggle='tooltip' title='" . $requests_count . " Request(s)'><i style='font-size:0.4em; line-height:30px; vertical-align:top' class='fa fa-circle' aria-hidden='true'></i><span>";
                            $calendar_html = $calendar_html . '</div></div>';
                        }
                    }
                    else {
                        if ($col == 0 || $col == 6) {
                            $calendar_html = $calendar_html . '<div style="float:left; width:14.28%; text-align:center; height:50px; line-height:50px; color:red;"></div>';
                        } else {
    ...
    public function render_equipment_calendar($month, $year, $calendar_days, $equipment_calendar) {
        ...
        if (isset($calendar_days)) {
            ...
            $calendar_html = $calendar_html . '<thead>';
            $calendar_html = $calendar_html . '<tr>';
            $calendar_html = $calendar_html . '<th style="width: 14.28%"><div class="text-center">Sunday2</div></th>';
            $calendar_html = $calendar_html . '<th style="width: 14.28%"><div class="text-center">Monday</div></th>';
            $calendar_html = $calendar_html . '<th style="width: 14.28%"><div class="text-center">Tuesday</div></th>';
            $calendar_html = $calendar_html . '<th style="width: 14.28%"><div class="text-center">Wednesday</div></th>';
            $calendar_html = $calendar_html . '<th style="width: 14.28%"><div class="text-center">Thursday</div></th>';
    ...
    public function render_equipment_calendar_new($equipment_id, $month, $year, $calendar_days, $equipment_calendar, $equipment_booking_calendar, $booking_request_calendar, $equipment_block_calendar = false) {
        $dateObj = DateTime::createFromFormat('!m', $month);
        $monthName = $dateObj->format('F');
        ...
        $this->CI->load->library('validation');
        $nextbookingtoken = $this->CI->validation->gen_token(array($equipment_id, $next_month, $next_year, "Calen"));
        $previousbookingtoken = $this->CI->validation->gen_token(array($equipment_id, $previous_month, $previous_year, "Calen"));
        /*         * *************** */

        $calendar_html = "";
        $calendar_html = $calendar_html . '<div class="row" style="border-bottom:1px solid #9e9e9e">';
        $calendar_html = $calendar_html . '<div class="col-md-1" style="float:left; font-size:1.3em; text-align:center; height:40px; line-height:40px; color:#757575"><a href="' . $this->CI->config->item('base_url') . 'master/equipment/calendar/' . $previous_month . '/' . $previous_year . '/' . $equipment_id . '/' . $previousbookingtoken . '"><i class="fa fa-angle-left" style="float:left; font-size:1.3em; text-align:center; height:40px; line-height:40px; color:#757575" aria-hidden="true" data-toggle="tooltip" title="Prev month"></i></a></div>';
        $calendar_html = $calendar_html . '<div class="col-md-10" style="float:left; font-size:1.3em; text-align:center; height:40px; line-height:40px; color:#757575">';
        $calendar_html = $calendar_html . strtoupper($monthName) . "  " . $year;
        $calendar_html = $calendar_html . '</div>';
        $calendar_html = $calendar_html . '<div class="col-md-1" style="float:left; font-size:1.3em; text-align:center; height:40px; line-height:40px; color:#757575"><a href="' . $this->CI->config->item('base_url') . 'master/equipment/calendar/' . $next_month . '/' . $next_year . '/' . $equipment_id . '/' . $nextbookingtoken . '"><i class="fa fa-angle-right" style="float:left; font-size:1.3em; text-align:center; height:40px; line-height:40px; color:#757575" aria-hidden="true" data-toggle="tooltip" title="Next month"></i></a></div>';
        $calendar_html = $calendar_html . '</div>';
        if (isset($calendar_days)) {
            $calendar_html = $calendar_html . '<div class="row">';
            $calendar_html = $calendar_html . '<div style="float:left; width:14.28%; font-size:1.1em; font-weight:400; text-align:center; height:50px; line-height:50px; color:red" data-toggle="tooltip" title="Sunday">S</div>';
            ...
            for ($row = 0; $row < 6; $row++) {
                ...
                for ($col = 0; $col < 7; $col++) {
                    if (isset($calendar_days[$row][$col])) {
                        ...
                        $blocks = false;
                        $blocks_count = 0;
                        if ($equipment_block_calendar) {
                            foreach ($equipment_block_calendar as $equipment_block_entry) {
                                $block_day = $equipment_block_entry["block_day"];
                                if ($block_day == $day_number) {
                                    $blocks = true;
                                    $blocks_count++;
                                }
                            }
                        ...
                        if ($down_date) {
                            ...
                            if ($requests)
                                $calendar_html = $calendar_html . "<span style='color:orange' data-toggle='tooltip' title='" . $requests_count . " Request(s)'><i style='font-size:0.4em; line-height:30px; vertical-align:top' class='fa fa-circle' aria-hidden='true'></i><span>";
                            if ($blocks)
                                $calendar_html = $calendar_html . "<span style='color:gray' data-toggle='tooltip' title='" . $blocks_count . " Block(s)'><i style='font-size:0.4em; line-height:30px; vertical-align:top' class='fa fa-circle' aria-hidden='true'></i><span>";
                            $calendar_html = $calendar_html . '</div></div>';
                        }
                    }else {
                        if ($col == 0 || $col == 6) {
