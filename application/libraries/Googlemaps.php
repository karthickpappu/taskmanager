...
//	var $apiKey				= 'AIzaSyCqWEFsP8OZsciiHTSjLh26XOozAkA0X10'; 						// If you've got an API key you can use it by passing this parameter. Setup an API key here: https://code.google.com/apis/console
        var $apiKey = 'AIzaSyAeRo_BiAiWsAW4RScAmMCx9agqynpASvY'; // If you've got an API key you can use it by passing this parameter. Setup an API key here: https://code.google.com/apis/console
	...
	var $map_height				= "420px";					// The height of the map container. Any units (ie 'px') can be used. If no units are provided 'px' will be presumed
	...
	var $region				= 'IN';						// Country code top-level domain (eg "uk") within which to search. Useful if supplying addresses rather than lat/longs
...
function create_map_supplier(){
		...
		if ($this->maps_loaded == 0)
		{
			...
			if ($this->cluster) { $this->output_js .= '
			<script type="text/javascript" src="' . ( ($this->https) ? 'https' : 'http' ) . 
'://google-maps-utility-library-v3.googlecode.com/svn/trunk/markerclusterer/src/markerclusterer_compiled.js"></script>
			'; }
		}
		if ($this->jsfile=="") {
			$this->output_js .= '
		...
		if (count($this->styles)) {
			...
			foreach ($this->styles as $style) {
				...
				if ($this->stylesAsMapTypes) {
					$this->output_js_contents .= 'var styles_'.$styles.' = new google.maps.StyledMapType(styles_'.$styles.', 
{name:"'.$style['name'].'"});
				';
				}else{
					$styleOutput .= $this->map_name.'.setOptions({styles: styles_'.$styles.'});
		...
		if ($this->mapTypeControlStyle!="" && (strtoupper($this->mapTypeControlStyle)=="DROPDOWN_MENU" || 
strtoupper($this->mapTypeControlStyle)=="HORIZONTAL_BAR")) {
			array_push($mapTypeControlOptions, 'style: google.maps.MapTypeControlStyle.'.strtoupper($this->mapTypeControlStyle));
		}
		if (count($this->map_types_available)) {
			foreach ($this->map_types_available as $map_type) { array_push($map_types, 'google.maps.MapTypeId.'.strtoupper($map_type)); }
		...
		$zoomControlOptions = array();
		if ($this->zoomControlPosition!="") { array_push($zoomControlOptions, 'position: 
google.maps.ControlPosition.'.strtoupper($this->zoomControlPosition)); }
...
array_push($zoomControlOptions, 'style: google.maps.ZoomControlStyle.'.strtoupper($this->zoomControlStyle)); }
		...
		if ($this->directions) {
			...
			if (trim($this->directionsUnits)!="" && (strtolower(trim($this->directionsUnits)) == "metric" || 
strtolower(trim($this->directionsUnits)) == "imperial")) {
				$this->output_js_contents .= ',unitSystem: google.maps.UnitSystem.'.strtoupper(trim($this->directionsUnits)).'
					';
			}
			if ($this->directionsAvoidTolls) {
			  	...
			  	directionsService.route(request, function(response, status) {
			    	if (status == google.maps.DirectionsStatus.OK) {
			    		...
			    		switch (status) {
			    			...
			    			case "MAX_WAYPOINTS_EXCEEDED": { alert("Maximum waypoints exceeded. Maximum of 8 allowed"); break }
			    			case "INVALID_REQUEST": { alert("Invalid request made for obtaining directions"); break }
			    			case "OVER_QUERY_LIMIT": { alert("This webpage has sent too many requests recently. Please try again later"); 
break }
			    			case "REQUEST_DENIED": { alert("This webpage is not allowed to request directions"); break }
			    			case "UNKNOWN_ERROR": { alert("Unknown error with the server. Please try again later"); break }
			    		}
		...
		$this->output_html .= '<div id="'.$this->map_div_id.'" style="width:'.$this->map_width.'; height:'.$this->map_height.';"'.(($this->class != "") 
