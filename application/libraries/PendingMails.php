    ...
    public function send_pending_mails() {
        ...
        foreach ($pending_mails as $row) {
            ...
            if ($dec_entity_code == 'IN') {
                ...
                $notif_message = $notif_message . "Guidelines : " . $this->CI->config->item('base_url') . "guidelines" . "<br>";
            ...
            } else if ($dec_entity_code == 'SA') {
                $this->CI->load->model('InstituteModel', '', TRUE);
                $this->CI->load->model('IstemUserModel', '', TRUE);
                $this->CI->load->model('supplier/SupplierModel', 'SupandServModel', TRUE);

                $user_name = $this->CI->SupandServModel->get_user_name($dec_user_id);
                $supplier_name = $this->CI->SupandServModel->get_supplier_name($dec_entity_ref_id);
                $supplier_id = $this->CI->SupandServModel->get_supplier_id($supplier_name);
                $supplier_data = $this->CI->SupandServModel->get_supplier_list($supplier_id);
                foreach ($supplier_data as $row) {
                    $facility = $row['facility_category'];
                }
                if ($facility == '41') {
                    $role_name = "Supplier";
                } elseif ($facility == '42') {
                    ...
                    $role_name = "Supplier and Service Provider";
                }
//send mail

                $subject = 'ISTEM Registration Confirmation - ' . $supplier_name;

                $notif_message = "<strong>Dear Sir/Madam,</strong><br>";
                $notif_message = $notif_message . "Guidelines : " . $this->CI->config->item('base_url') . "guidelines" . "<br>";
                $notif_message = $notif_message . "<P><h5>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . "     " . "Thank you for registering with One Nation One Portal for Linking Researchers and Resources, i.e., the Indian Science Technology and Engineering facilities Map (I-STEM). On behalf of the Citizens of India, the I-STEM Team, in association with Office of the Principal Scientific Adviser (PSA), and the Prime Minister Office (PMO), welcomes you and your team to the unique portal which is built to serve the scientific fraternity in a transparent and efficient manner.</h5></p>";
                $notif_message = $notif_message . "<h2>" . $role_name . ' ' . "Confirmation</h2><br>";
                $notif_message = $notif_message . "Name : <strong>" . $supplier_name . "</strong><br>";
                $notif_message = $notif_message . "<h3>Below are the login details</h3><br>";
                $notif_message = $notif_message . "Username : <strong>" . $user_name . "</strong><br>";
                $notif_message = $notif_message . "Password : <strong>" . $dec_password . "</strong><br>";
                $notif_message = $notif_message . "URL : " . $this->CI->config->item('base_url') . "<br>";
                ...
                $ip = $this->CI->logdetails->get_ip();
                $mail_from = $this->CI->config->item('super_admin_email');
                $mail_sub = 'Supplier Registration';
                $mail_content = $notif_message;
                $mail_data = array(
                    'mail_type' => $mail_type,
                    'mail_from' => $mail_from,
                ...
                $this->CI->load->model('supplier/MailBoxModel', 'SupplierMail', TRUE);
                $this->CI->SupplierMail->insert($mail_data);
                ...
                $user_name = $this->CI->InstituteModel->get_user_name($dec_user_id);
                $institute_id = $this->CI->InstituteModel->get_institute_name($dec_entity_ref_id);
                //  $institute_id = $this->CI->InstituteModel->get_institute_id($institute_name);
                $institute_data = $this->CI->InstituteModel->get_institutelist($institute_id);
                $bank_enrolment_status = $this->CI->InstituteModel->get_institutelist($institute_id);
                $institute_name = $institute_data['institute_name'];
                $update_data = array(
                    "bank_enrolment" => '3'   //processing                   
                );
                $this->CI->InstituteModel->update_bank_enrolment_processing_staus($institute_id, $update_data);
                //sending mail for billdesk
                $subject = 'ISTEM Bank ECS Form - ' . $institute_name;

                $payment_institute_id = $institute_id;
                $notif_message = " ";
                $notif_message = $notif_message . "ISTEM Bank ECS Form - " . $institute_data['institute_name'] . "<br>";
                //$notif_message = $notif_message . "Name : <strong>" . $institute_name . "</strong><br>";


                $notif_message = $notif_message . "Institution :  <strong>" . $payment_institute_id . "</strong><br>";
                ...
                $user_mobile_number = $this->CI->PublicUserModel->get_user_mobile_number($dec_user_id); //login using mobile number
                ...
                $notif_message = $notif_message . "<P><h5>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . "     " . "Thank you for registering with One Nation One Portal for Linking Researchers and Resources, i.e., the Indian Science Technology and Engineering facilities Map (I-STEM). On 
                            ...
                            and the Prime Minister Office (PMO), welcomes you and your team to the unique portal which is built to serve the scientific fraternity in a transparent and efficient manner.</h5></P>";
                $notif_message = $notif_message . "<p><h5>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . "     " . "To log in to the I-STEM portal, please choose the appropriate login credentials. Then, Create the content about your institution/department/Equipment/Project Status 
                            (or /Update/Edit content that has already been uploaded). Being in the highest position of â€œUser hierarchyâ€�, you will be able to create and manage following types of user accounts.</h5></p>";
                $notif_message = $notif_message . "<h5>1.&nbsp;&nbsp;Institute Representative(s): The Registrar/Principal/Director may create login credential and assign to his/her Representative(s) the responsibilities of looking after "
                        . "the activities of the Institution through Portal, such as, managing Booking Request, Equipment Health monitoring and gathering Usage Statistics, dealing with Grievances, etc. The login credential will be sent through email from the Portal.</h5>";
                $notif_message = $notif_message . "<p><h5>2.&nbsp;&nbsp;Funding Agency(ies),: Funding Agency means the Govt. Agency (DST, DRDO, DBT, etc.) that has provided the grants for procuring eqpt./facilities."
                        . " Your Institution/Organisation/Institution is requested to FIRST identify and create the funding agency for a GIVEN eqpt. and then upload the details for THAT equipment in specified format (i.e., Excel sheet) as per "
                        . "the guidelines provided on the page itself. In case funds have been provided by YOUR INSTITUTION, please identify the funding agency as â€œInstitutionâ€�.</h5>";
                $notif_message = $notif_message . "<p><h5>3.&nbsp;&nbsp;Department(s) and its admin(s): ONLY the Registrar/Principal/Director or his/her Representative has the authority to identify and create login for a Department Admin. "
                        . "Once the Admin of a Department is identified his login credential will be sent from the portal through email. The Admin would be able to create the login credentials for Faculty-in-charge,"
                        . " Faculty Coordinator, Facility(ies) and Operator(s)/Technologist(s)</h5>";
                $notif_message = $notif_message . "<p><h5>4.&nbsp;&nbsp;Faculty In-charge of facility (ies): ONLY the Department Head or his/her Representative/Admin may identify and create login for the Faculty-in-charge."
                ...
                $notif_message = $notif_message . "<p><h5>5.&nbsp;&nbsp;Facility Coordinator(s): ONLY the Faculty-in-charge or his/her Representative/Admin may identify and create the login for the Facility Coordinator. Once Facility Coordinator gets the "
                        . "login credential through email from the portal, can then create the login credentials for Facility(ies) and Operator(s)/Technologist(s).</h5>";
                $notif_message = $notif_message . "<p><h5>6.&nbsp;&nbsp;Facility(ies): All those in the hierarchy identified above may â€œcreate and enter dataâ€� for Department(s) and Facility (ies) established in an Institution, as per the guidelines provided on the page.</h5>";
                $notif_message = $notif_message . "<p><h5>7.&nbsp;&nbsp;Operator(s)/Technologist(s): S/he can add the Equipment and maintain the equipment booking calendar, approve requests (to use eqpt.) received from Internal/External Users,"
                        . " and fulfil the request (execute the job) AFTER receiving information of payment (if any).</h5>";
                $notif_message = $notif_message . "<p><h5>8.&nbsp;&nbsp;Public User(s) may register themselves to the portal (as explained on the portal) and send a request to book ANY equipment listed on the portal (that is needed for their R&D work)."
                        . " Once the relevant Equipment Technologist/Operator(s) approve their request, Users may plan to make a payment requested by the Equipment Technologist/Operator(s); the Users may then visit the Institute as per the schedule provided to them.</h5>";
               ...
               $notif_message = $notif_message . "I-STEM Mobile Number: <strong>" . $user_mobile_number . "</strong><br>";
                $notif_message = $notif_message . "I-STEM Password : <strong>" . $dec_password . "</strong><br>";
                $notif_message = $notif_message . "URL : " . $this->CI->config->item('base_url') . "<br>";
                $notif_message = $notif_message . "<strong>Note : Kindly enable/disable the notification in the profile section, if you do not want it in your email and continue checking the inbox at the I-STEM portal.</strong><br>";
               $notif_message = $notif_message . "<strong>You are now able to login to the I-STEM portal using your verified Mobile Number.</strong><br>";
                ...
                $mail_content = $notif_message;
                if ($user_data['user_name'] != "" || $user_data['user_name'] != Null) {
                    $mail_data = array(
                        'mail_type' => $mail_type,
                        'mail_from' => $mail_from,
                        'mail_to' => $mail_to,
                ...
                $user_name = $this->CI->PublicUserModel->get_user_name($dec_user_id);
                $contact_no = $this->CI->PublicUserModel->get_user_contact_number($user_name);
                $first_name = $this->CI->PublicUserModel->get_first_name($dec_user_id);
                $last_name = $this->CI->PublicUserModel->get_last_name($dec_user_id);
                $user_saluation = $this->CI->PublicUserModel->get_user_saluation($dec_user_id);

                $request_id = $this->CI->BookingRequestModel->get_requset_id_from_confirmation_id($dec_entity_ref_id);

                $booking_data = $this->CI->BookingRequestModel->get_booking_data($request_id);


                $user_id = $dec_user_id;
                //$institute_name = $this->CI->InstituteModel->get_institute_name($dec_entity_ref_id);
                //send mail
                $subject = 'I-STEM NOTIFICATION';
                $notif_message = "<strong> FBR &nbsp;" . $booking_data['fbr_no'] . "&nbsp; : &nbsp; Slot allocated on" . "&nbsp; " . $booking_data['confirmed_date'] . " " . $booking_data['confirmation_time_from'] . " to " . $booking_data['confirmed_to_date'] . " " . $booking_data['confirmation_time_to'] . ",</strong><br>";
                $notif_message = $notif_message . " Action: Please pay the usage charge and carry the sample in person or send through the courier/speed post to the operator. ";


                $notif_message = $notif_message . "<br/>URL : " . $this->CI->config->item('base_url') . "<br>";
                $notif_message = $notif_message . "<strong>Note : Kindly enable/disable the notification in the profile section, if you do not want it in your email and continue checking the inbox at the I-STEM portal.</strong><br>";
                // $notif_message = $notif_message . "URL : " . $this->config->item('base_url') . "<br>";
                ...
                //send mail
                $subject = 'I-STEM NOTIFICATION';
                $notif_message = "<strong>Reallocated ! FBR &nbsp;" . $booking_data['fbr_no'] . "&nbsp;is Rescheduled at " . "&nbsp; " . $booking_data['confirmed_date'] . " " . $booking_data['confirmation_time_from'] . " to " . $booking_data['confirmed_to_date'] . " " . $booking_data['confirmation_time_to'] . ",</strong><br>";
                $notif_message = $notif_message . " Action: Please wait for Operator for the confirmation of the slot time and date. ";
                ...
                $user_saluation = $this->CI->PublicUserModel->get_user_saluation($dec_user_id);
                ;
                $request_id = $this->CI->BookingRequestModel->get_requset_id_from_reject_id($dec_entity_ref_id);
                $booking_data = $this->CI->BookingRequestModel->get_booking_data($request_id);
                //$institute_name = $this->CI->InstituteModel->get_institute_name($dec_entity_ref_id);
                //send mail
                $subject = 'I-STEM NOTIFICATION';
                $notif_message = "<strong>Sorry! FBR  &nbsp;" . $booking_data['fbr_no'] . "&nbsp;is rejected.</strong><br>Equipment:" . $booking_data['equipment_name'] . "  Instituton:  " . $booking_data['institute_name'];
                $notif_message = $notif_message . "<br/><strong>Reason:&nbsp;&nbsp;" . $booking_data['rejection_reason'] . "";
                $notif_message = $notif_message . "<br/><strong>Status :&nbsp;&nbsp;" . "Rejected" . "";

                $notif_message = $notif_message . "<br/>URL : " . $this->CI->config->item('base_url') . "<br>";
                $notif_message = $notif_message . "<strong>Note : Kindly enable/disable the notification in the profile section, if you do not want it in your email and continue checking the inbox at the I-STEM portal.</strong><br>";

                $notif_message = $notif_message . "<br/>Please contact to Operator/Technologist for more information through Portal www.istem.gov.in . I-STEM Coordinator, Toll Free No. 1800-425-3281";
                // $notif_message = $notif_message . "URL : " . $this->config->item('base_url') . "<br>";
                ...
                //send mail
                $subject = 'I-STEM NOTIFICATION';
                $notif_message = "<strong>Opted for offline payment &nbsp;&nbsp; FBR &nbsp;" . $booking_data['fbr_no'] . "<br/>Slot allocated on" . "&nbsp; " . $booking_data['confirmed_date'] . " " . $booking_data['confirmation_time_from'] . " to " . $booking_data['confirmed_to_date'] . " " . $booking_data['confirmation_time_to'] . ",</strong><br>";
                $notif_message = $notif_message . " Please contact to Operator/Technologist for more information through Portal www.istem.gov.in . I-STEM Coordinator, Toll Free No. 1800-425-3281 ";
                ...
                //send mail
                $subject = 'I-STEM NOTIFICATION';
                $notif_message = "<strong>Payment Done Successfully. Your booking has been confirmed &nbsp;&nbsp; FBR &nbsp;" . $booking_data['fbr_no'] . "&nbsp;is scheduled at &nbsp;" . $booking_data['confirmed_date'] . $booking_data['confirmation_time_from'] . ':00&nbsp;at&nbsp;-' . $booking_data['confirmation_time_to'] . ':00&nbsp;' . '' . ",</strong><br>";
                $notif_message = $notif_message . " Please contact to Operator/Technologist for more information through Portal www.istem.gov.in . I-STEM Coordinator, Toll Free No. 1800-425-3281 ";
                ...
                //$institute_name = $this->CI->InstituteModel->get_institute_name($dec_entity_ref_id);
                //send mail
                $subject = 'I-STEM  || Query to operator';
                $notif_message = "<strong>Dear Sir/Madam,</strong><br>";
                $notif_message = $notif_message . "<P><h5>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . "     " . "Your query has been submitted,Soon you will get a response.</h5></P>";
...
To log in to the I-STEM portal, please choose the appropriate login credentials. Then, Create the content about your institution/department/Equipment/Project Status (or /Update/Edit content that has already been uploaded). Being in the highest position of â€œUser hierarchyâ€�, you will be able to create and manage following types of user accounts.
...
6.       Facility(ies): All those in the hierarchy identified above may â€œcreate and enter dataâ€� for Department(s) and Facility (ies) established in an Institution, as per the guidelines provided on the page.
...
9.       Funding Agency Admin: Funding Agencies of the GoI may also create login credentials for different Section-in-charge to look after the status of the equipment funded by the Agencies, e.g., as eqpt. health (Up/Down/Under repair), Usage statistics. Through the I-STEM Portals, Section-in-charge may also request the PI of the project (under which a given eqpt. has been procured) to submit the Project Technical Report that may be due, etc. The login credential will sent to all the section incharge through the portal.
            ...
            try {
                ...
                $this->CI->mailer->sendmail($dec_email_id, $subject, $notif_message, $cc, $attachments, $path);
